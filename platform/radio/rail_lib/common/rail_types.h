...
/**
 * Option to select Antenna 0 for transmission. This option has priority
 * over \ref RAIL_TX_OPTION_ANTENNA1. If not antenna selection option is set
 * then the TX will occur on whichever antenna was selected by the RX
 * options.
 */
#define RAIL_TX_OPTION_ANTENNA0 (1UL << RAIL_TX_OPTION_ANTENNA0_SHIFT)
/**
 * Option to select Antenna 1 for transmission. Option
 * \ref RAIL_TX_OPTION_ANTENNA0 has priority over this option. If not
 * antenna selection option is set then the TX will occur on whichever
 * antenna was selected by the RX options.
 */
...
typedef struct RAIL_RxPacketDetails {
  ...
  uint8_t subPhyId;
  /**
   * For configurations where the device has multiple antennae, indicates
